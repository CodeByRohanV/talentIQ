...
import { testAPI, candidatesAPI } from '@/lib/api';
import { Clock, FileQuestion, Loader2, AlertCircle, UserPlus, Info, CheckCircle2 } from 'lucide-react';
import { z } from 'zod';
...
export default function TestLanding() {
  ...
  const [view, setView] = useState<'details' | 'instructions'>('details');
  ...
  const fetchAssessment = async () => {
    try {
      ...
      if (response.requiresRegistration) {
        ...
        // If it's already a candidate token, redirect straight to taking the test
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: err.message || 'Failed to start the assessment. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setSubmitting(false);
        ...
        <Card className="max-w-md w-full">
          <CardContent className="pt-8 text-center">
            ...
            <p className="text-muted-foreground">
              {isNotStarted ? (
                <>
                  This test is scheduled to start on {new Date(assessment?.availableFrom!).toLocaleString()}. 
